...
// iOS bloquea el audio si no se activa con un toque directo del usuario.
// Se llama desde los botones (Empezar, Reanudar) para "despertar" el sonido.
...
// Sustituye a window.confirm() — en iOS, las apps instaladas en pantalla de
// inicio (modo standalone) no muestran el confirm() nativo del navegador.
...
function tick() {
  ...
  saveCounter++;
  if (saveCounter >= 12) { // cada ~3s (12 x 250ms)
    saveCounter = 0;
    saveProgress();
  }
}
...
/* ============ PERSISTENCIA LOCAL DE LA SESIÓN EN CURSO ============ */
// Guarda el progreso en el propio móvil (no en Firebase) para poder
// recuperarlo si iOS mata la app en segundo plano.
const PROGRESS_KEY = "cronohitos_sesion_activa";
...
async function offerResumeIfNeeded() {
  ...
  if (!saved) return;
  const name = saved.activeTemplateName || "Sesión libre";
  const ok = await confirmAction(`Tenías una sesión sin terminar ("${name}"). ¿Quieres continuarla donde la dejaste?`);
  if (!ok) { clearProgress(); return; }

  state.milestones = saved.milestones;
  state.currentIndex = saved.currentIndex;
  state.accumulatedMs = saved.accumulatedMs;
  state.fired = saved.fired;
  state.log = saved.log;
  state.sessionStartedAt = saved.sessionStartedAt;
  state.activeTemplateName = saved.activeTemplateName;

  // recalculamos dónde empezó el hito actual a partir de lo ya completado
  state.segmentStartMs = saved.log.reduce((s, m) => s + m.actualSec * 1000, 0);

  document.getElementById("setup-panel").classList.add("hidden");
  document.getElementById("run-panel").classList.remove("hidden");
  renderMilestoneTrack();
  if (saved.running) startTimer(); else tick();
}
...
async function finishSession() {
  ...
  try {
    ...
    showToast("No se pudo guardar en el historial (se queda a salvo en el móvil, prueba de nuevo)", "warn");
  }
...
async function renderTemplates() {
  ...
  state.templates.forEach(t => {
    ...
    card.querySelector(".del-btn").addEventListener("click", async () => {
      const ok = await confirmAction(`¿Borrar la plantilla "${t.name}"?`);
      if (!ok) return;
...
async function renderHistory() {
  ...
  sessions.forEach(s => {
    ...
    card.innerHTML = `
      ...
      `).join("")}
      <div class="card-actions">
        <button class="btn btn-ghost btn-danger del-session-btn">Borrar sesión</button>
      </div>
    `;
    card.querySelector(".del-session-btn").addEventListener("click", async () => {
      const ok = await confirmAction(`¿Borrar esta sesión de "${s.templateName}" del historial?`);
      if (!ok) return;
      try {
        await FirebaseAPI.deleteSession(s.id);
        showToast("Sesión borrada", "info");
        renderHistory();
      } catch (e) {
        console.error(e);
        showToast(`Error: ${e.message || e.code || e}`, "warn");
      }
    });
    list.appendChild(card);
  });
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  document.getElementById("finish-btn").addEventListener("click", async () => {
    const ok = await confirmAction("¿Terminar la sesión ahora?");
    if (ok) finishSession();
  });
  ...
  document.getElementById("test-sound-btn").addEventListener("click", () => {
    ...
    try {
      ...
      showToast(ok ? "Audio activo — ¿lo has oído?" : `Estado del audio: ${audioCtx ? audioCtx.state : "no disponible"}`, ok ? "success" : "warn");
    } catch (e) {
      showToast("No se pudo activar el audio en este dispositivo", "warn");
    }
    vibrate([100]);
  });
